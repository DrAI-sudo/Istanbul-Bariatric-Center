            ...
            <li key={year} className="flex flex-col items-center group" role="listitem">
              <div className="w-24 h-28 md:w-28 md:h-32 relative transition-transform duration-300 group-hover:scale-110" aria-label={`WhatClinic Service Award ${year}`}>
                <div className="w-full h-full bg-white rounded-t-lg relative overflow-hidden shadow-lg" style={{
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center p-2 border-4 border-amber-700" style={{
                    clipPath: 'polygon(0 0, 100% 0, 100% 85%, 50% 100%, 0 85%)'
                  }}>
                    <span className="text-amber-800 text-[10px] md:text-xs font-bold tracking-widest">SERVICE</span>
                    <span className="text-amber-800 text-lg md:text-xl font-extrabold">AWARD</span>
                    <span className="text-amber-800 text-xl md:text-2xl font-extrabold">{year}</span>
                  </div>
                </div>
              </div>
            </li>
